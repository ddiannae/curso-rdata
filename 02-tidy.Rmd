...
Tidyverse es una colección de paquetes de R diseñados para ciencia de datos. Todos comparten
una misma filosofía, gramática y estructura de datos lo que facilita su uso. Además, cada uno
contiene funciones para realizar tareas concretas. Por ejemplo: `tidyr` contiene funciones para 
obtener tidy datasets, `dplyr` permite manipular datos, `readr` leer datos tabulares, etc. Una 
descripción completa de los paquetes puede encontrarse en: [https://www.tidyverse.org/packages/](https://www.tidyverse.org/packages/){target="_blank"}. 

## Tidy data

Uno de los puntos principales en la filosofía de los paquetes de tidyverse es que se trabaja con 
datasets tidy. Esto significa que cumplen las siguientes tres reglas interconectadas:
...
El paquete `readr`, con sus funciones `read_csv` o `read_tsv` es comúnmente utilizado para importar
archivos con datos a tibbles en R. Sin embargo, el paquete `vroom` realiza la lectura de grandes
...
Para leer datos solamente ejecutamos la función `vroom()`

```{r message=FALSE}
anuies <- vroom::vroom("data/anuies.tsv")
anuies
```
Especificar el tipo de datos de alguna variable es útil cuando lo sabemos previamente. `vroom` y `readr`
leen los primeros mil valores para asignar el tipo de datos. Por ejemplo, cuando tenemos una columna 
...
Aunque son una herramienta poderosa, no podrán ser usados en todos los casos. En particular, debemos 
evitar su uso si las funciones dentro del pipe tienen múltiples entradas y múltiples salidas, ya que los pipes se 
usan cuando es un solo objeto el que está siendo transformado. 
Además, si se concatena un gran número de operaciones, se recomienda crear objetos intermedios, para 
que nuestro código sea fácil de leer. 
